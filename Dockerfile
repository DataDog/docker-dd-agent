...
# Install the Agent
RUN echo "deb http://apt.datadoghq.com/ beta main" > /etc/apt/sources.list.d/datadog.list \
 && apt-key adv --keyserver keyserver.ubuntu.com --recv-keys C7A7DA52 \
 && apt-get update \
 && apt-get install --no-install-recommends -y datadog-agent=1:5.9.0~rc.8-1 \
 && apt-get clean \
...
RUN /opt/datadog-agent/embedded/bin/curl -L -k -o /opt/datadog-agent/agent/checks.d/docker_daemon.py https://raw.githubusercontent.com/DataDog/dd-agent/master/checks.d/docker_daemon.py
